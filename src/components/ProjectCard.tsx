...
function StackBadge({ tech }: { tech: string }) {
  ...
  return (
    <span className={`font-mono text-[11px] font-bold px-3 py-1 bg-white border border-gray-200 rounded-sm shadow-sm ${brandColor}`}>
      {tech}
    </span>
  );
}
...
const GithubIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 inline-block mr-1.5 -mt-0.5">
  ...
  return (
    <article className="group relative flex flex-col border border-gray-200 bg-white hover:border-gray-300 hover:shadow-xl transition-all duration-300 rounded-sm overflow-hidden">
      ...
      <Link href={`/projects/${project.slug}`} className="absolute inset-0 z-0">
        <span className="sr-only">Ver detalles de {project.title}</span>
      </Link>

      <div className="relative aspect-[16/10] w-full bg-white overflow-hidden border-b border-gray-100 pointer-events-none">
        {project.image ? (
          <Image
            src={project.image}
      ...
      <div className="relative z-10 flex flex-col mt-auto border-t border-gray-50 bg-white">
        <div className="flex flex-wrap items-center gap-2 px-6 pt-5 pb-3">
          {project.demo && (
            <a
              ...
              className="text-[11px] font-mono font-bold text-gray-950 hover:text-[#F5B30A] transition-colors bg-gray-50 px-3 py-1.5 rounded-sm border border-gray-200 flex items-center"
            >
              Demo ↗
            </a>
          )}
          {project.github && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[11px] font-mono font-bold text-gray-600 hover:text-gray-950 transition-colors bg-gray-50 px-3 py-1.5 rounded-sm border border-gray-200 flex items-center"
            >
              <GithubIcon /> GitHub
            </a>
          )}
